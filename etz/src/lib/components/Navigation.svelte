<script lang="ts">
	...
	const navItems = [
		...
		{ href: '/mshiko', label: 'Mshiko', icon: 'history' },
...
<nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 safe-area-bottom">
	<div class="flex items-center justify-around py-2 px-4">
		{#each navItems as item}
			<a
				href={item.href}
				class="flex flex-col items-center gap-1 py-2 px-3 rounded-lg transition-colors {isActive(item.href, $page.url.pathname)
					? 'text-emerald-600'
					: 'text-gray-500'}"
			>
				{#if item.icon === 'home'}
