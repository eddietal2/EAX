<script lang="ts">
	...
	import { rates as liveRates, getStaticRates, fetchExchangeRates, initRatePolling, lastUpdated, isLoading, fetchError, clearFetchError } from '$lib/stores/exchangeRates';
...
<div bind:this={pageRef} class="h-full pt-8 bg-white dark:bg-gray-950 transition-colors duration-200 overflow-auto">
	...
	<div class="max-w-4xl mx-auto p-4 md:p-8 bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden {$isLoading ? 'opacity-75 pointer-events-none' : ''} transition-colors duration-200">
		<div class="border-b border-gray-100 dark:border-gray-800 -mx-4 md:-mx-8 px-4 md:px-8" bind:this={headerRef}>
			<div class="flex flex-col gap-2 mb-4">
				...
				{#if $fetchError}
					<div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded px-3 py-2 flex items-start gap-2">
						...
						<p class="text-xs text-yellow-800 dark:text-yellow-200 flex-1">{$fetchError}</p>
						<button
							type="button"
							onclick={clearFetchError}
							class="flex-shrink-0 p-1 -m-1 rounded-md text-yellow-700 dark:text-yellow-300 hover:bg-yellow-100 dark:hover:bg-yellow-900/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-500/60 transition-colors"
							aria-label="Dismiss"
						>
			...
			<div class="flex flex-wrap items-center justify-center gap-x-3 gap-y-2 mb-8 py-3 px-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
				<span class="text-sm text-gray-500 dark:text-gray-400">{t('market.subtitle')}</span>
				<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-100 dark:bg-green-900/40 border border-green-200 dark:border-green-800">
					<FlagIcon code={selectedToCurrencyCode} size="sm" />
					<span class="font-semibold text-green-800 dark:text-green-300 text-sm">{selectedToCurrencyCode}</span>
					<span class="text-[10px] font-bold uppercase tracking-wide text-green-700 dark:text-green-400">TO</span>
				</span>
				<button
				...
				<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-100 dark:bg-blue-900/40 border border-blue-200 dark:border-blue-800">
					<FlagIcon code={selectedFromCurrencyCode} size="sm" />
					<span class="font-semibold text-blue-800 dark:text-blue-300 text-sm">{selectedFromCurrencyCode}</span>
					<span class="text-[10px] font-bold uppercase tracking-wide text-blue-700 dark:text-blue-400">FROM</span>
				</span>
			</div>
		</div>
